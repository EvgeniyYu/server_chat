...
void RequestExecutor::set_request(TypeFunc type, unsigned _session_handler, const std::string& data)
{
	...
	if (type == NEW_CONNECTION)
		request = std::make_unique<AuthenticateUser>(session_handler, chat);
...
AuthenticateUser::AuthenticateUser(unsigned _session_handler, std::shared_ptr<UserChat> _chat): chat(_chat)
{
	this->session_handler = _session_handler;
}

void AuthenticateUser::execute()
{
	//message to send
	MessageData msg;
	User user;
