...
class UserChat
{
	...
	std::unique_ptr<IDatabaseAPI> db;
	std::map<unsigned, User> users;		//session_handle, user_name
	std::map<unsigned, std::vector<std::string>> rooms;	//room_id, vector_of_user_names
public:
	UserChat(std::shared_ptr<IServerAPI> _server_socket);
